...
오른쪽 아래에 <b>「저장 확인됨」</b>이 떠야 진짜로 들어간 것입니다. 몇 초 걸릴 수 있습니다.<br>
...
function 확인실패문구(r) {
  ...
  return "학번 · 이름 · 개별 비밀번호가 맞지 않습니다. 선생님께 개별 비밀번호를 확인하세요";
}
...
function 보내기(w, data, final) {
  저장함 = false;   /* 서버가 받았다고 확인될 때까지는 아직 저장 안 된 것으로 봅니다 */
  ...
  note("");
  $("state").textContent = (final ? "제출하는 중… " : "저장하는 중… ") + t;
  setTimeout(function () { 저장확인(data, final, t, 5); }, 3000);
}

/* ── 정말 들어갔는지 서버에 다시 물어봅니다 ──
   POST 는 응답을 볼 수 없습니다. 이 확인이 없으면 서버가 못 받았어도
   화면에는 「저장했습니다」가 뜹니다. 사람이 몰려 밀리는 중일 수 있으니
   몇 번 더 물어보고 나서 실패로 봅니다. */
function 저장확인(data, final, t, 남은시도) {
  $("state").textContent = (final ? "제출 확인 중… " : "저장 확인 중… ") + t;
  서버에묻기(function (r) {
    const 다시 = function (까닭) {
      if (남은시도 > 0) { setTimeout(function () { 저장확인(data, final, t, 남은시도 - 1); }, 8000); return; }
      저장함 = false;
      $("state").innerHTML = '<b class="bad">' + 까닭 + '</b> — 한 번 더 눌러 주세요';
    };
    if (!r || !r.ok) { 다시("저장을 확인하지 못했습니다"); return; }

    const 서버 = r.fields || {};
    const 안맞는칸 = KEYS.filter(function (k) {
      const v = String(data[k] == null ? "" : data[k]).trim();
      if (!v || v === "－") return false;
      return String(서버[k] == null ? "" : 서버[k]).trim() !== v;
    });
    if (안맞는칸.length) { 다시("저장이 덜 됐습니다 (" + 안맞는칸.length + "칸)"); return; }

    저장함 = true;
    제출됨 = (r.status === "제출");
    $("state").innerHTML = (final ? "<b>제출 확인됨</b> — " : "<b>저장 확인됨</b> — ")
      + t + (r.when ? " <span style='font-weight:400'>(서버 " + r.when + ")</span>" : "");
  });
}
